...
module.exports = (bot) => {
  ...
  bot.onText(/\/start/, async (msg) => {
    ...
    // Rate limiting check
    if (rateLimiter.isRateLimited(userId, 5, 60000)) {
      // 5 start commands per minute
    ...
    try {
      ...
      const loadingMessage = await messageHandler.safeSendMessage(
        bot,
        chatId,
      ...
      const welcomeText = `🎉 Welcome to Fullducks, ${firstName}!\n\n👤 Account: @${username}\n✅ Connected to your account\n\nChoose an option below:`;
  ...
  bot.onText(/\/wallet/, async (msg) => {
    ...
    // Rate limiting check
    if (rateLimiter.isRateLimited(userId, 10, 60000)) {
      // 10 wallet checks per minute
      await messageHandler.safeSendMessage(
        bot,
        chatId,
  ...
  bot.onText(/\/deposit/, async (msg) => {
    ...
    // Rate limiting check
    if (rateLimiter.isRateLimited(userId, 8, 60000)) {
      // 8 deposit commands per minute
      await messageHandler.safeSendMessage(
        bot,
        chatId,
  ...
  bot.onText(/\/status/, async (msg) => {
    ...
    // Rate limiting check
    if (rateLimiter.isRateLimited(userId, 3, 60000)) {
      // 3 status commands per minute
      await messageHandler.safeSendMessage(
        bot,
        chatId,
  ...
  bot.onText(/\/clear/, async (msg) => {
    ...
    try {
      ...
      await messageHandler.safeSendMessage(
        bot,
        chatId,
        "🗑️ Your session has been cleared.",
        keyboards.mainMenu,
      );
    } catch (error) {
